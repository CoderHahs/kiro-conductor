import BetterDatabase = require('better-sqlite3');
...
export class Database {
  ...
  private db: BetterDatabase.Database | null = null;
  ...
  public getDbPath(customPath?: string): string {
    if (customPath) {
      return customPath;
    }
  ...
  public init(dbPath?: string): BetterDatabase.Database {
    if (this.db) {
      return this.db;
    }

    const actualPath = this.getDbPath(dbPath);
    ...
    this.db = new BetterDatabase(actualPath);
  ...
  public getDB(): BetterDatabase.Database {
    if (!this.db) {
  ...
  // Typed query helpers
  public getOne<T>(sql: string, params: any[] = []): T | undefined {
  ...
  public run(sql: string, params: any[] = []): BetterDatabase.RunResult {
    const statement = this.getDB().prepare(sql);
...
// Preserve backwards compatibility for the existing tests/code that expect functional exports
export function getDbPath(customPath?: string): string {
  return Database.getInstance().getDbPath(customPath);
}

export function initDB(dbPath?: string): BetterDatabase.Database {
  return Database.getInstance().init(dbPath);
}

export function getDB(): BetterDatabase.Database {
...
export function getOne<T>(sql: string, params: any[] = []): T | undefined {
  return Database.getInstance().getOne<T>(sql, params);
}

export function getAll<T>(sql: string, params: any[] = []): T[] {
  return Database.getInstance().getAll<T>(sql, params);
}

export function run(sql: string, params: any[] = []): BetterDatabase.RunResult {
  return Database.getInstance().run(sql, params);
}
